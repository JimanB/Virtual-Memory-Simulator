...
#define PTE_SIZE 4 //standard size for a Page Table Entry in bytes
...
int main(int argc, char *argv[]) {
    ...
    int offset_bits = log2_int(page_size_bytes);
    int vpn_bits = address_bits - offset_bits;
    unsigned long long total_pages = 1ULL << vpn_bits;
    unsigned long long page_table_size = total_pages * PTE_SIZE;
    ...
    if (type == 2) {
        ptes_per_page = page_size_bytes / PTE_SIZE;
        pt_bits = log2_int(ptes_per_page);
        pd_bits = vpn_bits - pt_bits;
